...
const { errors, isLoading, editAudience, availableTypes } = useAudiences();

const state = reactive({
    description: props.audience.description,
    type: props.audience.type,
})
...
async function onSubmit() {
    ...
    await editAudience(props.audience.id, { description: state.description, type: state.type.name });
  ...
  <div class="fixed top-0 left-0 w-full h-full">
    <div class="w-[500px] top-1/4 left-[35%] p-7 max-w-full flex flex-col items-center rounded-md bg-primary-blue-900 text-white">
            <header class="w-full flex items-center justify-between">
                <h3 class="text-start font-semibold">Editar audiencia</h3>
            ...
            <form @submit.prevent="onSubmit" class="mt-5 w-full" >
                ...
                <div class="h-11 mb-3">
                    <Button class="button h-11" type="submit" rounded label="Editar" :loading="isLoading" />
                </div>
                <Toast class="mx-auto" position="bottom-center" />
                <Message v-if="success" severity="success">Modificado con éxito</Message>
