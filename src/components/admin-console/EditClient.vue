...
const { errors, isLoading, editClient } = useClients();

const state = reactive({
    fullName: props.client.fullName,
    email: props.client.email,
    file: props.client.file,
...
async function onSubmit() {
    ...
    await editClient(props.client.id, state);
  ...
  <div class="fixed top-0 left-0 w-full h-full">
    <div class="w-[500px] top-1/4 left-[35%] p-7 max-w-full flex flex-col items-center rounded-md bg-primary-blue-900 text-white">
            <header class="w-full flex items-center justify-between">
                <h3 class="text-start font-semibold">Editar cliente</h3>
